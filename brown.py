  ...
  for line in f:
    ...
    if (len(words)): # ASSERT len words == len psos
      for i in range(0,len(words)):
        ...
        if (i < len(words)-2):
          wwwFreq[ (words[i], words[i+1], words[i+2]) ] += 1
...
print ("Files are organized a sentence per line, noting some lines are blank, with words and parts of speech separated by a slash.")
print ("The filenames correspond to categories, defined in cats.txt file.")

# category stats
f1 = open("cFreq", "w")
for cat in numSentencesCat:
  f1.write(cat + "\t" + str(numFilesCat[cat]) + "\t" + str(numSentencesCat[cat]) + "\t" + str(numWordsCat[cat]) + "\n")
...
f1 = open("wwFreq", "w")
f2 = open("wwFreqNoSpace", "w")
for (word1, word2) in wwFreq:
  f1.write(word1 + "\t" + word2 + "\t" + str(wwFreq[ (word1, word2) ]) + "\n")
  f2.write(word1 + word2 + "\t" + str(wwFreq[ (word1, word2) ]) + "\n")
print ("wwFreq is written to disk")
f1.close

#trigrams
f1 = open("wwwFreq", "w")
f2 = open("wwwFreqOnly", "w")
f3 = open("wwwFreqNoSpace", "w")
for (word1, word2, word3) in wwwFreq:
  f1.write(word1 + "\t" + word2 + "\t" +  word3 + "\t" + str(wwwFreq[ (word1, word2, word3) ]) + "\n")
  f2.write(str(wwwFreq[ (word1, word2, word3) ]) + "\n")
  f3.write(word1 + word2 + word3 + "\t" + str(wwwFreq[ (word1, word2, word3) ]) + "\n")
print ("wwwFreq is written to disk")
f1.close
...
for (pos1, pos2) in ppFreq:
  ...
  #f2.write(pos1 + pos2 + "\t" + str(ppFreq[ (pos1, pos2) ]) + "\n")
...
f1 = open("wpFreq", "w")
f2 = open("wpFreqNoSpace", "w")
for (word, pos) in wpFreq:
  f1.write(word + "\t" + pos + "\t" + str(wpFreq[ (word, pos) ]) + "\n")
  f2.write(word + pos + "\t" + str(wpFreq[ (word, pos) ]) + "\n")
